...
/*
session：服务端会话机制（服务器使用一种类似散列表结构来保存信息）

C端->S端（判断C端请求中是否有Session ID）-》有则检索session，没有则创建session

S端创建session，并生成session id，返回C端保存

session & cookie目的：解决http协议无状态缺陷（不指道请求是否属于同一客户端，不保存状态，无记忆）

session & cookie完成方法不同：
1。session通过cookie,在C端保存session id,而将用户的其他会话消息保存在S端的Session对象中
2。cookie需要将所有信息都保存在C端
*/

/*
GO语言如何使用session

session是在S端实现的一种用户和服务器间认证的解决方案

目前GO标准包未提供Session支持，自己实现Session管理和创建
*/
/*
session管理器:https://studygolang.com/articles/12856
Session劫持防范：
https://www.jianshu.com/p/d52f40ac39d0
